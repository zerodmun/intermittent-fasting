...
## [0.8.0] — 2026-07-29
### Added
- **Offline-First, Reliable, Self-Healing Notification Scheduler Architecture**: Completed complete architectural audit and redesign of `NotificationService`. Operates 100% offline via native `AndroidScheduleMode.exactAllowWhileIdle` alarms. Treats `AlarmManager` strictly as a write-only execution target with Database (Today's Fasting Schedule) as the primary Source of Truth. Implements a 10-step deterministic scheduling lifecycle (pre-validation, midnight rollover reset, target reminder generation, integrity checking, idempotency evaluation, differential rescheduling of obsolete alarms only, independent error isolation, and immediate cache synchronization). Added explicit `ScheduledNotificationReceiver` and `ScheduledNotificationBootReceiver` declarations in `AndroidManifest.xml` for `BOOT_COMPLETED` and `MY_PACKAGE_REPLACED` intents. Mapped user sound preferences to 3 dedicated Notification Channel IDs (`fasting_reminders`, `fasting_reminders_app`, `fasting_reminders_silent`). Implemented on-demand `NotificationDiagnostics` API.
- **Settings Menu Layout UI Refactoring**: Reorganized main `SettingsScreen` layout by displaying `Notification` (with single card `Notification Settings`) and `Widgets & Persistent Notification` (with single card `Widgets & Persistent Notification`) as separate, dedicated top-level sections. Removed the nested `Widgets & Persistent Notification` card from `NotificationSettingsScreen`.
- **Standalone Exercises & Workout Journal Module**: Expanded `ExerciseScreen` into a dedicated standalone workout log feature (`workout_log.dart`, `workout_providers.dart`, `workout_detail_screen.dart`, `add_edit_workout_sheet.dart`). Supports logging workout title, date, duration, dynamic exercise items (sets, reps, weight $\text{kg}$, notes), ACSM/Compendium MET calorie estimation with volume intensity adjustment, $\approx 70\%$ calculation accuracy badges, synchronized workout duration summary cards (`totalDurationMinutes`), and a simplified top-right `+ Workout` action button. Completely decoupled from fasting timers and other modules.
- **UI Consistency Across Data-Entry Screens**: Unified action button placement by removing redundant Floating Action Buttons (FABs) from **Total Calories Consumed** (`food_intake_summary_screen.dart`) and **Body Composition** (`body_comp_screen.dart`), replacing them with consistent top-right AppBar action text buttons with icons (`+ Food` and `+ Record`).
